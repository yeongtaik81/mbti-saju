      ...
      payload: JSON.stringify(
        {
          ...
          draft,
          instruction:
            'Return strict JSON only. Write in warm Korean, like a thoughtful woman in her 30s or 40s speaking to a friend. Do not stack isolated one-line statements. Each major section should read as 1 or 2 connected paragraphs, with 2 to 4 sentences per paragraph. Respect the selected scenario and make the reading feel specific to that situation rather than a generic category summary. If two scenarios belong to the same broad category, still make them clearly different in question, judgment criteria, and advice. For example, 재회 가능성, 다시 연락 올까, 고백 타이밍 must not read like the same romance reading with minor wording changes. The upper visual section already shows technical saju data, so do not fill the body text with jargon such as 원국, 십성, 지장간, 12운성, 현금흐름, 포지셔닝, 리스크, 운영, 구조화. Use easy everyday language in the body. If you mention that an element is strong or weak, immediately explain what that means in daily life. Do not expose internal classifier labels or type names such as 누수관리형, 긴장성장형, 균형축적형, 신중거리형 그대로 in user-facing prose. Paraphrase their meaning in plain Korean instead. Avoid repeating the same key noun such as 흐름, 기준, 관계, 운 more than necessary inside one paragraph; vary the wording naturally in plain Korean. If MBTI is missing, do not mention that it is missing or unavailable directly. Keep interpretation weighting as saju 70~80% and MBTI 20~30%. Blend MBTI naturally across the result instead of creating a standalone MBTI section, and connect MBTI and saju in the same paragraph. Use correct Korean particles after names such as 은/는 and 이/가. Do not mechanically open every major section with MBTI. For every self scenario except 기본 해석, and for all compatibility scenarios, summary and overview must open with the selected question or relationship situation itself, not with MBTI type names, 성향, or 기질. In those cases narrativeFlow, timingHint, and caution also should not start with MBTI; MBTI may appear later in the paragraph only if it adds nuance. summary must act like a short judgment line, while overview must add the next layer of explanation instead of repeating the same opening claim. Focus each scenario on its own decision point and give roughly premium-detail density instead of collapsing back into a short generic answer. Use clear, confident phrasing such as "~합니다", "~편입니다", "~하기 쉬운 흐름입니다". Avoid vague hedging like "가능성이 있습니다". Give practical and specific advice. actions must contain exactly 3 items. Keep JSON schema: {summary, sectionsJson:{overview,coreSignal,caution,actions,reflectionQuestion,storyTitle?,sajuEvidence?,sajuBasis?,subjectLens?,narrativeFlow?,tenYearFlow?,currentDaewoon?,yearlyFlow?,wealthFlow?,relationshipFlow?,pairDynamic?,attractionPoint?,conflictTrigger?,communicationTip?,relationshipLens?,careerMoneyLens?,timingHint?}}.'
        },
        null,
        2
      ),
      ...
      payload: JSON.stringify(
        {
          ...
          content,
          instruction:
            'Return strict JSON only. Review this saju result for production quality. Approve only if the result is warm, easy for a general user to read, uses connected short paragraphs instead of one-line fragments, explains strong or weak elements in everyday meaning when they appear, avoids business wording and heavy saju jargon in the body text, keeps saju-first reasoning with MBTI as a secondary refinement layer, does not mechanically force MBTI into every opening sentence, uses correct Korean particles after names, does not expose internal classifier labels such as 누수관리형 or 긴장성장형 그대로, avoids repeating the same key noun such as 흐름, 기준, 관계, 운 too often inside one paragraph, does not mention missing MBTI information directly, and feels specific to the selected scenario rather than reading like a generic category template. For every self scenario except 기본 해석, and for all compatibility scenarios, reject the result if summary or overview opens with MBTI type names, 성향, or 기질 instead of the actual question or relationship situation. Also reject if narrativeFlow, timingHint, or caution starts with MBTI for those scenarios. Also reject if summary and overview feel like the same opening claim said twice. Reject results that feel corporate, repetitive, overly technical, too fragmented, vague, or grammatically awkward around 조사. Schema: {approved:boolean, issues:string[], score:number}.'
        },
        null,
        2
      ),
